#!/usr/bin/env bash
CONTAINER_NAME='ubuntu-yocto-build-18.04'
...
${BIN} run --memory-swappiness=0 --cpuset-cpus=${CPU_CORES} --memory=${RAM_MEMORY} --user ${CONTAINER_USER} -it -v ${CONTAINER_USER_LOCAL_HOME}:/home/${CONTAINER_USER}:Z -e DISPLAY=unix$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix ${CONTAINER_NAME} /bin/bash
